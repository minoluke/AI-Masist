# MASist - LLMによるマルチエージェントシミュレーション研究基盤

## 概要

MASist（Multi-Agent Simulation with LLM Intelligent Systems Tool）は、大規模言語モデル（LLM）を用いたマルチエージェントシミュレーション（MAS）の研究基盤です。自然言語によるシミュレーション設計から、実験実行、結果分析、論文執筆までを一貫して支援します。
...
5年先、AIが物理世界を理解し始めると、以下のような領域でMASによる将来予測が主流になると予想されます：
...
1. **シンプルな系での振る舞い検証**: ゲーム理論的設定での行動分析
...
export S2_API_KEY="your-semantic-scholar-api-key"  # 任意
```
...
| ドキュメント | 説明 |
|-------------|------|
| [MASIST_IDEA_FORMAT.md](../docs/MASIST_IDEA_FORMAT.md) | アイデアJSONフォーマット定義 |
| [PERFORM_IDEATION_PROMPTS.md](../docs/PERFORM_IDEATION_PROMPTS.md) | アイデア生成プロンプト |
| [IDEA_FORMAT_IMPACT_ANALYSIS.md](../docs/IDEA_FORMAT_IMPACT_ANALYSIS.md) | フォーマット変更の影響分析 |
| [LAUNCH_MASIST_MIGRATION_PLAN.md](../docs/LAUNCH_MASIST_MIGRATION_PLAN.md) | AI-Scientistからの移行計画 |
