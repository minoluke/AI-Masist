...
reviewer_system_prompt_base = (
    "You are an AI researcher who is reviewing a paper that was submitted to a prestigious multi-agent simulation venue."
...
def get_fewshot_examples_path():
    """Get the path to fewshot_examples directory using importlib.resources."""
    try:
        # Python 3.9+
        ...
        return pkg_resources.resource_filename('masist', 'fewshot_examples')
...
def get_review_fewshot_examples(num_fs_examples=1):
    fewshot_prompt = """
Below are some sample reviews, copied from previous AI/ML conferences.
...
"""
    fewshot_base_path = get_fewshot_examples_path()

    for paper_name, review_name in zip(
        fewshot_papers[:num_fs_examples], fewshot_reviews[:num_fs_examples]
    ):
        try:
            # Try to access via importlib.resources for packaged installations
            try:
                # Python 3.9+
                with importlib.resources.as_file(fewshot_base_path.joinpath(paper_name)) as paper_path:
                    txt_path = str(paper_path).replace(".pdf", ".txt")
                ...
                with importlib.resources.as_file(fewshot_base_path.joinpath(review_name)) as review_path:
                    review_text = load_review(str(review_path))
            except AttributeError:
                # Python 3.7-3.8 fallback
                paper_path = os.path.join(str(fewshot_base_path), paper_name)
                review_path = os.path.join(str(fewshot_base_path), review_name)
...
"""
            ...
            print(f"Warning: Could not load fewshot example {paper_name}: {e}")
            continue

    return fewshot_prompt


meta_reviewer_system_prompt = """You are an Area Chair at an AI research conference.
