...
def get_citation_addition(
    ...
    msg_history = []
    citation_system_msg_template = """You are an ambitious AI researcher who is looking to publish a paper to a workshop at ICLR 2025 that explores real-world pitfalls, failures, and challenges in multi-agent systems and social simulations.
...
def perform_writeup(
    ...
    try:
        ...
        try:
            vlm_client, vlm_model_name = create_vlm_client(vlm_model)
            desc_map = {}
            for pf in plot_names:
                ...
                review_data = generate_vlm_img_review(img_dict, vlm_model_name, vlm_client)
                if review_data:
        ...
        if not latex_code_match:
            print("ERROR: Could not extract LaTeX code block from LLM response.")
        ...
        for i in range(n_writeup_reflections):
            ...
            review_img_cap_ref = perform_imgs_cap_ref_review(
                vlm_client, vlm_model_name, current_pdf
            )
            ...
            reflection_page_info = get_reflection_page_info(current_pdf, page_limit)
...
"""
            ...
            # AI-Scientist-v2 と同じ: "I am done" チェック
            ...
            review_img_selection = perform_imgs_cap_ref_review_selection(
                vlm_client, vlm_model_name, current_pdf, reflection_page_info
            )
...
If you believe you are done with reflection, simply say: "I am done"."""
        ...
        # Get the last compiled PDF for page info
        last_pdf = base_pdf_file + f"_{compile_attempt-1}.pdf" if compile_attempt > 0 else pdf_file

        # Get new reflection_page_info
        reflection_page_info = get_reflection_page_info(last_pdf, page_limit)
...
USE MINIMAL EDITS TO OPTIMIZE THE PAGE LIMIT USAGE."""
        ...
        if reflection_code_match:
            reflected_latex_code = reflection_code_match.group(1).strip()
            # current_latex might not be defined if loop exited early, read from file
            with open(writeup_file, "r") as f:
                current_latex_content = f.read()
            if reflected_latex_code != current_latex_content:
                final_text = reflected_latex_code
                cleanup_map = {
                    "</end": r"\\end",
                    "</begin": r"\\begin",
        ...
        # AI-Scientist-v2 と同じロジック: 最後にコンパイルしたPDFの存在をチェック
