...
class AgentManager:
    def __init__(self, task_desc: str, cfg: Any, workspace_dir: Path):
        ...
        self.main_stage_goals: Dict[int, str] = {
            1: """
                - エンドツーエンドで動く最小実装を完成させる（仮説→シミュレーション実行→結果要約→論文ドラフト冒頭まで）
                - ごくシンプルなシナリオ（例：2エージェントの交渉・協力ゲーム・役割対立など）でマルチエージェント対話シミュレーションを動かす
                - ロール定義 / 対話プロトコル / 終了条件 / ログ形式 を「1つの型」として最後まで通す""",
            2: """
                - 実験条件のバリエーションを増やして、結果の安定性・妥当性を高める
                - シミュレーションの基本構造（エージェントフレームワークやプロトコルの大枠）は変えずに、以下を系統的に変える：
                  - プロンプト条件（フレーミング、ルール説明の仕方など）
                  - 環境パラメータ（報酬構造、情報の非対称性、人数、ルールなど）""",
            3: """
                - 新しい改善を探索
                - 新しい知見を得るための実験を考案
                - 創造的に、既存の枠にとらわれずに考える""",
            4: """
                - 各要素の貢献度を明らかにする系統的分析フェーズ
                - Stage 2〜3 で使用した同じシナリオセットを使用""",
        }
