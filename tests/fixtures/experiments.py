...
最大反復数：
デフォルト T = 30 ターン（技術採用が飽和するまでの時間を想定）。
...
背景・文脈
近年、複数のLLMエージェントを同一環境で同時に動かす「LLMマルチエージェント・シミュレーション」が増えている。しかし、多くは「人間 vs AI」であり、「AI vs AI」の協力・非協力行動の体系的理解は不足している。本研究は、行動経済学の反復公共財ゲームをLLMどうしにプレイさせ、「自己認識（相手が自分自身と明示される状況）」が協力行動をどう変えるかを調べる。

目的
LLMに「あなたは別のAIとプレイしている（No-Name）」と伝える場合と、「あなたと同じモデル名のAIとプレイしている（Name）」と伝える場合で、平均寄与額がどのように異なるかを評価する。
...
仮説
仮説1（自己認識効果）
 Name 条件では No-Name より平均寄与額が体系的に変化する（減るモデル・増えるモデルがある）。

仮説2（プロンプトとの交互作用）
collective では Name 条件が協力を減らしやすい。
selfish では Name 条件が協力を増やしやすい。

仮説3（ロバスト性）
2人・4人の人数設定で定性的に再現する。

その他（任意）
Study 1〜3 でプロンプト文・ルール提示頻度・推論出力の有無を操作することで、効果がプロンプト依存ではないことを検証している。
...
エージェントの状態更新
- 各ラウンドのフィードバック（総寄与額・利得・累積得点）を履歴として記憶
- 次ラウンドの入力プロンプトとして履歴を渡す
- 内部戦略更新はモデルが暗黙に行う

エージェントと環境との相互作用
- ラウンド開始：環境 → エージェントに情報提示
...
各工程の試行数
Study 1 & 2：条件あたり約 100 ゲーム
Study 3：条件あたり約 50 ゲーム

フェーズ構造（任意）
Study 1：毎ラウンドでルール再提示＋推論出力あり
Study 2：プロンプト言い換え＋ルール再提示なし
Study 3：4 人ゲーム、推論なし

対話フロー
- システムプロンプト＝ルール・モデル名・プロンプト属性
- ユーザーメッセージ＝履歴＋寄与の要求
...
ルール
共有情報
- ラウンド数・利得計算式・初期ポイント・乗数
- ラウンド終了後の総寄与額・自身の利得
- Name / No-Name の説明文（プレイヤーに提示される）

非公開情報
- 実際には Name 条件でも「自己対戦ではなく、別の API 呼び出し」であること（モデルには不可視）
- 他プレイヤーに割り当てたシステムプロンプトの情報は非公開でも公開でもよい

意思決定ルール
- LLM が履歴とプロンプトを参照して 0〜10 の整数を自由に選択
- Study 1 のみ推論テキストを生成

利得構造
payoff_i(t) = (10 − C_i(t)) + 1.6 × T(t) / N
最終得点最大化が形式的な目標

実験条件
- 自己認識：Name / No-Name
- プロンプト：collective / neutral / selfish
- Study：1,2,3（プロンプト構造・推論有無・人数の違い）
...
ログ形式
特に指定なし
後からゲーム×ラウンド×プレイヤー単位で復元できる構造にする
...
仮説の検証方法
仮説1：
 Name / No-Name の平均寄与額を統計比較（t検定・混合効果モデルなど）。

仮説2：
 寄与額を従属変数とし、要因（自己認識 × プロンプト）の交互作用の有意性を分析。

仮説3：
 モデル別・人数別で同様の比較を行い、効果のロバスト性を評価。
"""
