...
def test_return_selection_info():
    """Test 9: return_selection_info=True returns selection details"""
    print("\n" + "=" * 80)
    print("Test 9: return_selection_info=True")
    print("=" * 80)
    ...
    node1 = create_test_node(
        "node_good",
        metric_value=0.7,
        plan="Proper experimental design",
        code="# Good code",
    )
    node2 = create_test_node(
        "node_other",
        metric_value=0.8,
        plan="Another approach",
        code="# Other code",
    )
    ...
    mock_response = {
        "selected_id": "node_good",
        "reasoning": "Node1 has better hypothesis testing methodology",
        "hypothesis_validation": "Provides clear evidence with proper condition comparison",
        "scientific_soundness": 4,
    }

    task_desc = "Test if rules increase cooperation"

    with patch("masist.treesearch.journal.query", return_value=mock_response):
        # Test with return_selection_info=True
        best_node, selection_info = journal.get_best_node(
            use_val_metric_only=False,
            task_desc=task_desc,
        ...
        assert best_node is not None, "Should return a node"
        assert best_node.id == "node_good", f"Expected node_good, got {best_node.id}"

        # Verify selection_info contains all fields
        assert selection_info is not None, "Should return selection_info"
        assert selection_info["method"] == "llm_selection", f"Expected llm_selection, got {selection_info['method']}"
        assert selection_info["selected_id"] == "node_good"
        assert selection_info["reasoning"] == mock_response["reasoning"]
        assert selection_info["hypothesis_validation"] == mock_response["hypothesis_validation"]
        assert selection_info["scientific_soundness"] == 4

    print(f"✓ Node returned: {best_node.id}")
    print(f"✓ Selection method: {selection_info['method']}")
    print(f"✓ Reasoning: {selection_info['reasoning']}")
    print(f"✓ Hypothesis Validation: {selection_info['hypothesis_validation']}")
    print(f"✓ Scientific Soundness: {selection_info['scientific_soundness']}/5")
    print("✅ Test 9 PASSED")
    return True


def test_return_selection_info_with_real_llm():
    """Test 10: Real LLM with return_selection_info (integration test)"""
    print("\n" + "=" * 80)
    print("Test 10: Real LLM with return_selection_info")
    print("=" * 80)
...
""",
    ...
    try:
        ...
        print(f"Metric: {best_node.metric.value}")
        print(f"\nMethod: {selection_info['method']}")
        print(f"\nReasoning:\n  {selection_info.get('reasoning', 'N/A')}")
        print(f"\nHypothesis Validation:\n  {selection_info.get('hypothesis_validation', 'N/A')}")
        print(f"\nScientific Soundness: {selection_info.get('scientific_soundness', 'N/A')}/5")
        print(f"{'='*60}\n")

        # Verify that hypothesis_validation was evaluated
        assert "hypothesis_validation" in selection_info, "Should have hypothesis_validation"
        assert selection_info["hypothesis_validation"], "hypothesis_validation should not be empty"

        # Verify scientific_soundness is a valid score
        soundness = selection_info.get("scientific_soundness")
        assert soundness is not None, "Should have scientific_soundness"
        assert 1 <= soundness <= 5, f"scientific_soundness should be 1-5, got {soundness}"

        print("✓ hypothesis_validation is present and non-empty")
        print(f"✓ scientific_soundness is valid: {soundness}/5")
...
def run_all_tests():
    ...
    tests = [
        ...
        ("Test 9: return_selection_info", test_return_selection_info),
        ("Test 10: Real LLM with selection_info", test_return_selection_info_with_real_llm),
    ]
